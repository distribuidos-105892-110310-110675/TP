...
class FilterItemsByYear:
    ...
    def __handle_sigterm_signal(self, signal, frame):
        logging.info("Received SIGTERM, shutting down FilterItemsByYear")
        self.running = False
    ...
    def start(self):
        ...
        while i < len(ITEMS):
            ...
            self.__filter_by_year(communication_protocol.encode_transaction_items_batch_message(new_chunk))
            i += chunk_size
    ...
    def __filter_by_year(self, chunk):
        ...
        filtered = []
        for i in items:
            year = int(i['created_at'].split(' ')[0].split('-')[0])
