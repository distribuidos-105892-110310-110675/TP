...
TRANSACTIONS = [
    ...
    {"transaction_id": "t-007", "store_id": 1, 'user_id': '', 'final_amount': 82.9, "created_at": "2025-10-06 10:06:50"},
...
class MapMonthYearTransactions:
    def __init__(self):
        self.running = True
    ...
    def __handle_sigterm_signal(self, signal, frame):
        logging.info("Received SIGTERM, exiting MapMonthYearTransactions")
        self.running = False
    ...
    def start(self):
        ...
        while i < len(TRANSACTIONS):
            ...
            self.__map_year_to_month(communication_protocol.encode_transactions_batch_message(new_chunk))
            i += chunk_size
        self.__map_year_to_month(
            communication_protocol.encode_eof_message(communication_protocol.TRANSACTIONS_BATCH_MSG_TYPE))

    def __map_year_to_month(self, chunk):
        ...
        for t in transactions:
            ...
            month = (date.split('-')[1])
            year = (date.split('-')[0])
            t['year_month_created_at'] = year + '-' + month
            mapped.append(t)
