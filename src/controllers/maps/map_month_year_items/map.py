...
class MapMonthYearItems:
    ...
    def __handle_sigterm_signal(self, signal, frame):
        logging.info("Received SIGTERM, exiting MapMonthYearItems")
        self.running = False
    ...
    def start(self):
        ...
        while i < len(ITEMS):
            ...
            self.__map_year_to_month(communication_protocol.encode_transaction_items_batch_message(new_chunk))
            i += chunk_size
    ...
    def __map_year_to_month(self, chunk):
        ...
        for i in items:
            ...
            month = (date.split('-')[1])
            year = (date.split('-')[0])
            i['year_month_created_at'] = year + '-' + month
            mapped.append(i)
